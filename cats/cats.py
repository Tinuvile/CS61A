...
def accuracy(typed, source):
    ...
    if typed_length == 0:
        return 0.0 if source_length > 0 else 100.0

    accurate = 0
    for i in range(min(source_length, typed_length)):
        if typed_words[i] == source_words[i]:
            accurate += 1

    return 100.0 * accurate / typed_length
    # END PROBLEM 3
...
def wpm(typed, elapsed):
    ...
    return typed_length * 12.0 / elapsed
    # END PROBLEM 4
