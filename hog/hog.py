...
def boar_brawl(player_score, opponent_score):
    ...
    """
    # BEGIN PROBLEM 2
    opponent_tenth = (opponent_score // 10) % 10
    player_th = player_score % 10
    score_getted = abs(opponent_tenth - player_th) * 3
    return max(score_getted, 1)
    # END PROBLEM 2
...
def max_scoring_num_rolls(dice=six_sided, times_called=1000):
    ...
    for num_rolls in range(1, 11):
        ...
        if averaged_score > max_average_score:
            max_average_score = averaged_score
            best_num_rolls = num_rolls
        elif averaged_score == max_average_score:
